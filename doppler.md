...
| `TARGET_SAMPLE_PER_STRATUM` | config | `/snapshot` lists a UNIFORM random sample of at most this many tokens per stratum instead of the whole contracts (13/09: the full epoch-1 listing is ~850k tokens, 40 h and ~$46 of RPC per refresh, and died to transport). `0` = full listing. Test hunt: `15000` |
| `TARGET_REFRESH_WORKERS` / `TARGET_REFRESH_RETRIES` | config | parallel metadata reads (default 4) and retries with backoff on 429/5xx before a read counts as transport (default 3) |
| `TARGET_REFRESH_MAX_TRANSPORT_SHARE` | config | share of metadata reads that may be lost to transport before the build aborts as an outage (default `0.02`). Sampled mode over the public gateway: `0.25` — a 1,000-token sample is not hurt by losing a few percent |
| `TARGET_GATE_GREEN_MIN` / `TARGET_GATE_RED_MAX` | config | gate thresholds on the EFFECTIVE pool (entries × writability × uniqueness). Defaults `100000` / `20000` (ratified 04/09: 20× an attacker budget of ~5,000 guesses). A non-default gate is printed in `/status` and in every gate verdict |
| `TARGET_GATE_MAX_SHARE` / `TARGET_GATE_HARD_SHARE` | config | per-stratum concentration caps, soft (default `0.40`, exemptable) and hard (default `0.70`, binds everyone). `1.0` disables |
| `TARGET_CAP_EXEMPT` | config | strata the SOFT cap does not bind, comma-separated (default `tail2021`, ratified 05/09). An unknown stratum name refuses at boot |
